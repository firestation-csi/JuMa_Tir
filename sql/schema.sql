...
-- ----------------------------------------------------------------
-- Aufgaben einer Station
-- boolean = Ja/Nein für die Gruppe (NEIN → points FP)
-- count   = Zähler 0–max_count je Teilnehmer (Anzahl × points FP)
-- time    = Zeitwertung via Stoppuhr (floor((Ist−Soll)/Zeiteinheit) × zeitstrafe_fp)
-- ----------------------------------------------------------------
CREATE TABLE IF NOT EXISTS station_tasks (
    ...
    max_count        SMALLINT UNSIGNED NULL,          -- Obergrenze Stepper (count-Typ)
    sort_order       TINYINT       NOT NULL DEFAULT 0,
    sollzeit_sek     SMALLINT UNSIGNED NULL,          -- time-Typ: Zielzeit
    hoechstzeit_sek  SMALLINT UNSIGNED NULL,          -- time-Typ: Maximale Zeit
    zeitstrafe_fp    TINYINT  UNSIGNED NULL,          -- time-Typ: FP je Zeiteinheit
    zeiteinheit_sek  TINYINT  UNSIGNED NULL,          -- time-Typ: Sekunden je Einheit
    created_at       DATETIME      NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at       DATETIME      NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (id),
